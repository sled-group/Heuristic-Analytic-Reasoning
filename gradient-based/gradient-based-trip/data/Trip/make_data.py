...
def raw_dict_to_dataset(raw_dict, is_curated=False, exclude_multi_confl=False, is_train=True):
            ...
            if not(exclude_multi_confl) or check_no_multi_confl(object["stories"]):
                ...
                if is_train:
                    if have_entity_specific_confl:
                        ...
                        pair_confl_entity_states = []
                        for _ in range(len(stories_json_data["pair_entities"])):
                            pair_confl_entity_states.append(0)
                        pair_confl_entity_states[entity_index] = 1
                        stories_json_data["pair_confl_entity_states"] = pair_confl_entity_states
                        stories_json_data["pair_confl_attribute"] = attribute_index
                        stories_json_data["pair_confl_effect_state"] = effect_state
                        stories_json_data["pair_confl_precondition_state"] = precondition_state
                        pair_states_only_relevant = []
                        for _ in range(len(temp_implausible_story["entities"])):
                            sentences_state = []
                            for _ in range(len(temp_implausible_story["sentences"])):
                                sentences_state.append([[0] * 20, [0] * 20])
                            pair_states_only_relevant.append(sentences_state)
                        eff_sentence_idx = pair_confl_pairs[0][0] if pair_confl_pairs[0][0] < pair_confl_pairs[0][1] else pair_confl_pairs[0][1]
                        pre_sentence_idx = pair_confl_pairs[0][1] if pair_confl_pairs[0][0] < pair_confl_pairs[0][1] else pair_confl_pairs[0][0]
                        pair_states_only_relevant[entity_index][eff_sentence_idx][1][attribute_index] = effect_state
                        pair_states_only_relevant[entity_index][pre_sentence_idx][0][attribute_index] = precondition_state
                        stories_json_data["pair_states"] = pair_states_only_relevant
                        ...
                        pair_confl_entity_states = []
                        for _ in range(len(stories_json_data["pair_entities"])):
                            pair_confl_entity_states.append(-1)
                        stories_json_data["pair_confl_entity_states"] = pair_confl_entity_states
                        stories_json_data["pair_confl_attribute"] = -1
                        stories_json_data["pair_confl_effect_state"] = -1
                        stories_json_data["pair_confl_precondition_state"] = -1
                        pair_states_do_not_backward = []
                        for _ in range(len(temp_implausible_story["entities"])):
                            sentences_state = []
                            for _ in range(len(temp_implausible_story["sentences"])):
                                sentences_state.append([[9] * 20, [9] * 20])
                            pair_states_do_not_backward.append(sentences_state)
                        stories_json_data["pair_states"] = pair_states_do_not_backward
                        if have_all_states:
                            dataset.append(stories_json_data)
                else:
                    if is_curated:
...
def make_trip_dataset():
    ...
    trip_dataset["train"] = raw_dict_to_dataset(raw_train, is_curated=False, exclude_multi_confl=False, is_train=True)
    print("Loading dev data...")
    trip_dataset["dev_full"] = raw_dict_to_dataset(raw_dev, is_curated=False, exclude_multi_confl=False, is_train=False)
    trip_dataset["dev_curated"] = raw_dict_to_dataset(raw_dev, is_curated=True, exclude_multi_confl=False, is_train=False)
    print("Loading test data...")
    trip_dataset["test_full"] = raw_dict_to_dataset(raw_test, is_curated=False, exclude_multi_confl=False, is_train=False)
    trip_dataset["test_curated"] = raw_dict_to_dataset(raw_test, is_curated=True, exclude_multi_confl=False, is_train=False)

    print("# pairs of stories in train_trip data:", len(trip_dataset["train"]))
    print("# pairs of stories in dev_full_trip data", len(trip_dataset["dev_full"]))
    print("# pairs of stories in test_full_trip data:", len(trip_dataset["test_full"]))

    print("# pairs of stories in dev_curated_trip data", len(trip_dataset["dev_curated"]))
    print("# pairs of stories in test_curated_trip data:", len(trip_dataset["test_curated"]))
...
def write_to_files(trip_dataset):
    if os.path.isfile("train_trip.json"):
        print("(train_trip.json exists) Write process suspended. Please delete train_trip.json and try again.")
    else:
        for json_obj in trip_dataset["train"]:
            with open("train_trip.json", "a") as output_file:
                output_file.write(json.dumps(json_obj) + "\n")
    if os.path.isfile("dev_full_trip.json"):
        print("(dev_full_trip.json exists) Write process suspended. Please delete dev_full_trip.json and try again.")
    else:
        for json_obj in trip_dataset["dev_full"]:
            with open("dev_full_trip.json", "a") as output_file:
                output_file.write(json.dumps(json_obj) + "\n")
    if os.path.isfile("test_full_trip.json"):
        print("(test_full_trip.json exists) Write process suspended. Please delete test_full_trip.json and try again.")
    else:
        for json_obj in trip_dataset["test_full"]:
            with open("test_full_trip.json", "a") as output_file:
                output_file.write(json.dumps(json_obj) + "\n")
    if os.path.isfile("dev_curated_trip.json"):
        print("(dev_curated_trip.json exists) Write process suspended. Please delete dev_curated_trip.json and try again.")
    else:
        for json_obj in trip_dataset["dev_curated"]:
            with open("dev_curated_trip.json", "a") as output_file:
                output_file.write(json.dumps(json_obj) + "\n")
    if os.path.isfile("test_curated_trip.json"):
        print("(test_curated_trip.json exists) Write process suspended. Please delete test_curated_trip.json and try again.")
    else:
        for json_obj in trip_dataset["test_curated"]:
            with open("test_curated_trip.json", "a") as output_file:
                output_file.write(json.dumps(json_obj) + "\n")
